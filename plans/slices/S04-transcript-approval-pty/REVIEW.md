...
S04 пока не закрыт целиком. В этом коммите закрыт следующий proof target: `daemon-owned streaming transcript lane` поверх уже существующего first-turn round-trip.
...
Следующий честный runtime шаг уже отдельно: approvals, PTY и virtualization. Этот коммит их не симулирует.
