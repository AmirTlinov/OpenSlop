...
struct OpenSlopApprovalProbe {
    static func main() async {
        ...
        do {
            ...
            print("approval_contains_command=\(approvalContainsCommand) contains_done=\(containsDone) final_turn=\(finalTurnStatus)")
            ...
            guard approvalContainsCommand, containsDone, transcript.lastTurnStatus == "completed" else {
                fputs("OpenSlopApprovalProbe failed: approval metadata or final DONE state did not match expectation.\n", stderr)
                exit(EXIT_FAILURE)
            }
        } catch {
